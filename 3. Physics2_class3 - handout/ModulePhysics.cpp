...
bool PhysBody::Contains(int x, int y) const
{
	...
	for (b2Fixture* b = body->GetFixtureList(); b; b = b->GetNext()) {
		...
		if (body->GetFixtureList()->TestPoint({ PIXEL_TO_METERS(x), PIXEL_TO_METERS(y) })) {
...
int PhysBody::RayCast(int x1, int y1, int x2, int y2, float& normal_x, float& normal_y) const
{
	...
	for (b2Fixture* f = body->GetFixtureList(); f; f = f->GetNext())
	{
		...
		if (hit)
		{
			b2Vec2 hitPoint = output.fraction * (input.p2 - input.p1);
			ret = METERS_TO_PIXELS(sqrt((hitPoint.x*hitPoint.x) +( hitPoint.y * hitPoint.y )));
